...
class ComptadorViewModel : ViewModel(){
    ...
    // Definim un metode public per accedir al valor del LiveData
    ...
    fun decrementarComtador() {
        // Funció per decrementar el comptador
        _comptador.value = _comptador.value?.minus(1)
    }

    fun resetejarComtador() {
        //Funció per resetejar el comtador
        _comptador.value = 0
    }
}